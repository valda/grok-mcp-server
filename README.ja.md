...
X API はリソース単位での従量課金（投稿 1 件あたり $0.005、[pay-per-usage](https://docs.x.com/x-api/pricing)）だが、`x_search` は Grok が内部で何件の投稿を参照しても **ツール呼び出し 1 回単位** での定額課金。具体的な比較:

| ユースケース | grok-mcp-server | X API 直 |
|---|---|---|
| 投稿 1 件 + リプライ 50 件のスレッド取得 | 約 $0.01（ツール呼び出し 1 回 + トークン） | 約 $0.255（51 × $0.005） |
| トレンド検索で 100 件の投稿取得 | 約 $0.01 | 約 $0.50 |
