...
X は [公式 MCP サーバー (xmcp)](https://github.com/xdevplatform/xmcp) を提供しており、X API v2 の全機能を 120 以上の MCP ツールとして公開している（投稿、DM、いいね、フォロー、アナリティクスなど）。**書き込み操作**（投稿、リツイート、DM 送信）やアカウント管理が必要なら、公式サーバーを使おう。

grok-mcp-server は異なるアプローチを取る。すべてを **xAI の Grok API** 経由で処理する。Grok API には X Search がビルトインツールとして含まれているため:

- **X API 開発者アカウント不要** — xAI API キーだけで使える
- **AI による検索・分析** — Grok が結果を解釈・要約・分析してくれる（公式サーバーは生の API データを返すのみ）
- **構造化出力** — 任意の JSON Schema で必要なデータだけを抽出
- **チェーン検索** — 前回の検索結果を引き継いだ深掘り・要約ワークフロー
- **フルアーカイブ検索がティア制限なし** — 公式 X API でのフルアーカイブ検索には Pro（$5,000/月）以上が必要

### コスト比較（2026年4月時点）

| | grok-mcp-server | 公式 X API (xmcp) |
|---|---|---|
| 月額固定費 | **なし** | $200/月 (Basic) 〜 $5,000/月 (Pro) |
| 検索1回あたり | **約 $0.005** + トークン費 | 月額プランに含まれる、または $0.005/read (従量課金) |
| フルアーカイブ検索 | **利用可能**（Grok 経由） | Pro ($5,000/月) 以上が必要 |
| 月100回検索した場合 | **約 $0.50** | **$200+/月** (最低プラン) |
| 書き込み操作 | 非対応 | 対応 |

**検索・分析用途では、grok-mcp-server のコストは桁違いに低い**。最新の料金は [xAI Models and Pricing](https://docs.x.ai/developers/models) と [X API アクセスレベル](https://developer.x.com/en/portal/products) を参照。
